...
public sealed class Relay : BackgroundService
{
    ...
    private void HandleControl(NetPeer from, PeerState self, NetPacketReader reader)
    {
        ...
        switch (type)
        {
            ...
            default:
                SendError(from, "PROTOCOL_ERROR", $"Unknown control message {(byte)type}");
                break;
        }
    }
    ...
    private void HandleConnectIntent(NetPeer from, PeerState self, NetPacketReader reader)
    {
        ...
        {
            SendError(from, "TARGET_NOT_FOUND", $"Target '{targetId}' not connected");
            return;
        }

        if (!string.Equals(self.SchemaId, target.SchemaId, StringComparison.Ordinal))
        {
            SendError(from, "SCHEMA_MISMATCH", $"Schema mismatch: self={self.SchemaId}, target={target.SchemaId}");
            return;
        }
    ...
    private void SendError(NetPeer peer, string code, string message)
    {
        var w = new NetDataWriter();
