...
public class DevelopViewModel : ReactiveObject, IDisposable
{
    ...
    public DevelopViewModel(SimClient sim)
    {
        ...
        {
            this.RaiseAndSetIfChanged(ref _isRecording, !_isRecording);
            if (_isRecording)
            {
                trace = new();
        ...
        {
            this.RaiseAndSetIfChanged(ref _isPlaying, !_isPlaying);
            if (_isPlaying)
            {
                Freeze(true);
                trace ??= new();
                ...
                {
                    this.RaiseAndSetIfChanged(ref _isPlaying, false);
                    Freeze(false);
                });
            }
            else
    ...
    private IDisposable PopulateTreeAndAttach(SimClient sim, string path)
    {
        ...
        {
            this.RaiseAndSetIfChanged(ref _loaded, $"Failed to load {path} configuration");
            return Disposable.Empty;
        }

        var nodes = PopulateTree(sim, tree);
        foreach (var node in nodes) Nodes.Add(node);
        this.RaiseAndSetIfChanged(ref _loaded, $"Loaded {path} configuration");
...
public class Node : ReactiveObject, IDisposable
{
    ...
    public Node(SimClient sim, Definition def) : this(string.Empty, false)
    {
        ...
        {
            this.RaiseAndSetIfChanged(ref _isPulse, true);
            var t = new DispatcherTimer { Interval = duration };
            t.Tick += (_, _) =>
            {
                t.Stop();
                this.RaiseAndSetIfChanged(ref _isPulse, false);
            };
            t.Start();
        }
    }
